...
type FormAnexarServidorProps = { // (ou FormAnexarEstagiarioProps)
...
export default function FormAnexarServidor({
  ...
  const fetchDocumentos = async () => {
    ...
    try {
      const response = await api.get("/buscar/documentos", {
        params: { funcionario_id: servidor.id },
      });
  ...
  const handleSubmit = async () => {
    ...
    if (servidor?.id) {
      formData.append("funcionario_id", String(servidor.id));
    }
  ...
  return (
    <div className="w-full max-w-4xl">
      ...
      <h1 className="pb-8 text-4xl font-semibold text-sky-950">
        Gerenciar Anexos do Servidor: {servidor?.nome}
      </h1>
