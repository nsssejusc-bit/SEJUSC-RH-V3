      ...
      {filterServidores()?.map((servidor) => (
        ...
        >
          <div>
            ...
            <p className="text-lg text-slate-700">{servidor.secretaria_lotacao}</p>
          </div>
          <div className="flex gap-2">
            ...
            {checkbox === "ativos" && (
              <>
                ...
                >
                  {isLoading.load && isLoading.id === servidor.id && isLoading.action === "ficha"
                    ? "Gerando"
                    : "Ficha"}
                </Button>
                <Button
                    className="cursor-pointer rounded-full border-2 border-sky-950 px-4 py-1.5 text-sm font-bold tracking-wider text-sky-950 uppercase duration-200 ease-in hover:bg-sky-950 hover:text-sky-100"
                    onClick={() => handleArchiveServidor(servidor.id!)}
                >
                    {isLoading.load && isLoading.id === servidor.id && isLoading.action === "arquivar" ? "Inativando" : "Inativar"}
                </Button>
              </>
            )}
            ...
            {checkbox === "inativos" && (
                <Button
                    className="cursor-pointer rounded-full border-2 border-sky-950 px-4 py-1.5 text-sm font-bold tracking-wider text-sky-950 uppercase duration-200 ease-in hover:bg-sky-950 hover:text-sky-100"
                    onClick={() => handleActiveServidor(servidor.id!)}
                >
                    {isLoading.load && isLoading.id === servidor.id && isLoading.action === "desarquivar" ? "Ativando" : "Ativar"}
                </Button>
            )}
          </div>
        </div>
